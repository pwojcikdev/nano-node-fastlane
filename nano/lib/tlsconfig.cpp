...
nano::error read_tls_config_toml (boost::filesystem::path const & data_path_a, nano::tls_config & config_a, nano::nlogger & nlogger, std::vector<std::string> const & config_overrides)
{
	nano::error error;
...
#else
		nlogger.critical (nano::log::tag::tls, "HTTPS or WSS is enabled in the TLS configuration, but the node is not built with NANO_SECURE_RPC");
		std::exit (1);
#endif
	}
