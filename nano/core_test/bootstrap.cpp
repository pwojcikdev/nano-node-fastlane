...
TEST (bootstrap_processor, process_new)
{
	...
	auto node3 (std::make_shared<nano::node> (nano::test::get_available_port (), nano::unique_path (), system.logging, system.work));
...
TEST (bootstrap_processor, DISABLED_lazy_pruning_missing_block)
{
	nano::test::system system;
	nano::node_config config (nano::test::get_available_port (), system.logging);
	config.frontiers_confirmation = nano::frontiers_confirmation_mode::disabled;
...
TEST (bulk, DISABLED_genesis_pruning)
{
	...
	auto node2 (std::make_shared<nano::node> (nano::test::get_available_port (), nano::unique_path (), system.logging, system.work, node_flags));
	ASSERT_FALSE (node2->init_error ());
	node2->start ();
	nano::block_hash latest1 (node1->latest (nano::dev::genesis_key.pub));
	nano::block_hash latest2 (node2->latest (nano::dev::genesis_key.pub));
