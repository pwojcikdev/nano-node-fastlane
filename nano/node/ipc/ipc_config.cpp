...
nano::error nano::ipc::ipc_config::serialize_toml (nano::tomlconfig & toml) const
{
	...
	tcp_l.put ("io_threads", transport_tcp.io_threads, "Number of threads dedicated to TCP I/O.\ntype:uint64_t");
...
nano::error nano::ipc::ipc_config::deserialize_toml (nano::tomlconfig & toml)
{
	...
	if (tcp_l)
	{
		tcp_l->get ("io_threads", transport_tcp.io_threads);
		tcp_l->get ("allow_unsafe", transport_tcp.allow_unsafe);
		tcp_l->get ("enable", transport_tcp.enabled);
		tcp_l->get ("port", transport_tcp.port);
		tcp_l->get ("io_timeout", transport_tcp.io_timeout);
	}

	auto domain_l (toml.get_optional_child ("local"));
	if (domain_l)
	{
		domain_l->get ("io_threads", transport_domain.io_threads);
		domain_l->get ("allow_unsafe", transport_domain.allow_unsafe);
		domain_l->get ("enable", transport_domain.enabled);
		domain_l->get ("path", transport_domain.path);
		domain_l->get ("io_timeout", transport_domain.io_timeout);
	}

	auto flatbuffers_l (toml.get_optional_child ("flatbuffers"));
	if (flatbuffers_l)
	{
		flatbuffers_l->get ("skip_unexpected_fields_in_json", flatbuffers.skip_unexpected_fields_in_json);
		flatbuffers_l->get ("verify_buffers", flatbuffers.verify_buffers);
	}
