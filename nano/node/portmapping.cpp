...
void nano::port_mapping::refresh_devices ()
{
	if (!node.network_params.network.is_dev_network ())
	{
		...
		nano::log::tag::upnp, "UPnP local address {}, discovery: {}, IGD search: {}",
		...
		{
			node.nlogger.debug (nano::log::tag::upnp, "UPnP device url: {}, st: {}, usn: {}", i->descURL, i->st, i->usn);
		}
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
		{
			...
			if (add_port_mapping_error_l == UPNPCOMMAND_SUCCESS)
			{
				...
				node.nlogger.info (nano::log::tag::upnp, "UPnP {} {}:{} mapped to: {}",
				protocol.name,
...
bool nano::port_mapping::check_lost_or_old_mapping ()
{
	...
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		...
		if (!recent_lease)
		{
			...
			node.nlogger.info (nano::log::tag::upnp, "UPnP lease time getting old, remaining time: {}, lease time: {}, below the threshold: {}",
		...
		node.nlogger.debug (nano::log::tag::upnp, "UPnP {} mapping verification response: {}, external ip response: {}, external ip: {}, internal ip: {}, remaining lease: {}",
		protocol.name,
		verify_port_mapping_error_l,
		external_ip_error_l,
...
void nano::port_mapping::check_mapping_loop ()
{
	...
	if (upnp.devices != nullptr)
	{
		...
		{
			node.nlogger.info (nano::log::tag::upnp, "UPnP No need to refresh the mapping");
		}
	}
	else
	{
		// Bump logging level periodically
		node.nlogger.log ((check_count % 15 == 0) ? nano::log::level::info : nano::log::level::debug,
		nano::log::tag::upnp, "UPnP No IGD devices found");
	}
...
void nano::port_mapping::stop ()
{
	...
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			...
			{
				node.nlogger.info (nano::log::tag::upnp, "UPnP shutdown {} port mapping successful: {}:{}",
				protocol.name,
				protocol.external_address.to_string (),
