...
void nano::distributed_work::success (std::string const & body_a, nano::tcp_endpoint const & endpoint_a)
{
	...
	try
	{
		...
		if (!nano::from_string_hex (work_text, work))
		{
			...
			{
				node.nlogger.error (nano::log::tag::distributed_work, "Incorrect work response from {}:{} for root {} with diffuculty {}: {}",
		...
		{
			node.nlogger.error (nano::log::tag::distributed_work, "Work response from {}:{} wasn't a number: {}",
			nano::util::to_str (endpoint_a.address ()),
			endpoint_a.port (),
			work_text);
		}
	...
	{
		node.nlogger.error (nano::log::tag::distributed_work, "Work response from {}:{} wasn't parsable: {}",
		nano::util::to_str (endpoint_a.address ()),
		endpoint_a.port (),
		body_a);
	}
