...
void nano::rep_crawler::validate ()
{
	...
	for (auto const & i : responses_l)
	{
		...
		{
			node.nlogger.debug (nano::log::tag::repcrawler, "Ignoring vote from loopback channel: {}", channel->to_string ());
		...
		if (inserted)
		{
			node.nlogger.info (nano::log::tag::repcrawler, "Found representative {} at {}", vote->account.to_account (), channel->to_string ());
		}
		if (updated)
		{
