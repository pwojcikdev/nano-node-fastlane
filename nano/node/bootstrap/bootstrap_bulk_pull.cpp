...
nano::bulk_pull_client::~bulk_pull_client ()
{
	...
	if (expected != pull.end && !expected.is_zero ())
	{
		...
		node->nlogger.debug (nano::log::tag::bulk_pull_client, "Bulk pull end block is not expected {} for account {} or head block {}", pull.end.to_string (), pull.account_or_head.to_account (), pull.account_or_head.to_string ());
	}
	else
	{
...
void nano::bulk_pull_client::received_block (boost::system::error_code ec, std::shared_ptr<nano::block> block)
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_client, "Insufficient work for bulk pull block: {}", block->hash ().to_string ());
...
void nano::bulk_pull_account_client::request ()
{
	...
	node->nlogger.trace (nano::log::tag::bulk_pull_account_client, nano::log::detail::requesting_pending,
...
void nano::bulk_pull_account_client::receive_pending ()
{
	...
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		if (size_a == size_l)
		{
			...
			{
				node->nlogger.debug (nano::log::tag::bulk_pull_account_client, "Error while receiving bulk pull account frontier: {}", ec.message ());
		...
		{
			node->nlogger.debug (nano::log::tag::bulk_pull_account_client, "Invalid size: Expected {}, got: {}", size_l, size_a);
...
void nano::bulk_pull_server::send_next ()
{
	...
	if (block != nullptr)
	{
		...
		node->nlogger.trace (nano::log::tag::bulk_pull_server, nano::log::detail::sending_block,
...
void nano::bulk_pull_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_server, "Unable to bulk send block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_server, "Unable to bulk send not-a-block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_account_server::set_params ()
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_account_server, "Invalid bulk_pull_account flags supplied: {}", static_cast<uint8_t> (request->flags));
...
void nano::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_account_server, "Unable to bulk send block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_account_server::complete (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_account_server, "Unable to pending-as-zero: {}", ec.message ());
	}
}
