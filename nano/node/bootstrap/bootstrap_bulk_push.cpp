...
void nano::bulk_push_client::start ()
{
	...
	message, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			node->nlogger.debug (nano::log::tag::bulk_push_client, "Unable to send bulk push request: {}", ec.message ());
		}
	},
...
void nano::bulk_push_server::receive ()
{
	...
	else
	{
		...
		connection->socket->async_read (receive_buffer, 1, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
			...
			{
				node->nlogger.debug (nano::log::tag::bulk_push_server, "Error receiving block type: {}", ec.message ());
			}
		});
	}
}
...
void nano::bulk_push_server::received_type ()
{
	...
	switch (type)
	{
		...
		default:
		{
			node->nlogger.debug (nano::log::tag::bulk_push_server, "Unknown type received as block type");
			break;
		}
	}
}
...
void nano::bulk_push_server::received_block (boost::system::error_code const & ec, std::size_t size_a, nano::block_type type_a)
{
	...
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			{
				node->nlogger.debug (nano::log::tag::bulk_push_server, "Insufficient work for bulk push block: {}", block->hash ().to_string ());
		...
		{
			node->nlogger.debug (nano::log::tag::bulk_push_server, "Error deserializing block received from pull request");
		}
	}
}
