...
std::shared_ptr<nano::bootstrap_client> nano::bootstrap_connections::connection (std::shared_ptr<nano::bootstrap_attempt> const & attempt_a, bool use_front_connection)
{
	...
	{
		node.nlogger.debug (nano::log::tag::bootstrap, "Bootstrap attempt stopped because there are no peers");
...
void nano::bootstrap_connections::connect_client (nano::tcp_endpoint const & endpoint_a, bool push_front)
{
	...
	[this_l, socket, endpoint_a, push_front] (boost::system::error_code const & ec) {
		if (!ec)
		{
			this_l->node.nlogger.debug (nano::log::tag::bootstrap, "Connection established to: {}", nano::util::to_str (endpoint_a));
		...
		else
		{
			...
			{
				default:
					this_l->node.nlogger.debug (nano::log::tag::bootstrap, "Error initiating bootstrap connection to: {} ({})", nano::util::to_str (endpoint_a), ec.message ());
					break;
...
void nano::bootstrap_connections::populate_connections (bool repeat)
{
	...
	std::unordered_set<nano::tcp_endpoint> endpoints;
	{
		...
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				...
				{
					node.nlogger.debug (nano::log::tag::bootstrap, "Stopping slow peer {} (elapsed sec {} > {} and {} blocks per second < {})",
	...
	if (sorted_connections.size () >= (target * 2) / 3 && target >= 4)
	{
		...
		node.nlogger.debug (nano::log::tag::bootstrap, "Dropping {} bulk pull peers, target connections {}", drop, target);

		for (int i = 0; i < drop; i++)
		{
			auto client = sorted_connections.top ();

			node.nlogger.debug (nano::log::tag::bootstrap, "Dropping peer with block rate {} and block count {} ({})",
	...
	node.nlogger.debug (nano::log::tag::bootstrap, "Bulk pull connections: {}, rate: {} blocks/sec, bootstrap attempts {}, remaining pulls: {}",
...
void nano::bootstrap_connections::requeue_pull (nano::pull_info const & pull_a, bool network_error)
{
	...
	if (attempt_l != nullptr)
	{
		...
		else
		{
			...
			node.nlogger.debug (nano::log::tag::bootstrap, "Failed to pull account {} or head block {} down to {} after {} attempts and {} blocks processed",
