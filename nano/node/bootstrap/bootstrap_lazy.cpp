...
void nano::bootstrap_attempt_lazy::run ()
{
	...
	if (!stopped)
	{
		node->nlogger.info (nano::log::tag::bootstrap_lazy, "Completed lazy pulls");
	}
	if (lazy_has_expired ())
	{
		node->nlogger.info (nano::log::tag::bootstrap_lazy, "Lazy bootstrap attempt ID {} expired", id);
	}
	lock.unlock ();
	stop ();
	condition.notify_all ();
...
void nano::bootstrap_attempt_wallet::run ()
{
	...
	if (!stopped)
	{
		node->nlogger.info (nano::log::tag::bootstrap_lazy, "Completed wallet lazy pulls");
	}
	lock.unlock ();
	stop ();
	condition.notify_all ();
