...
void nano::frontier_req_client::run (nano::account const & start_account_a, uint32_t const frontiers_age_a, uint32_t const count_a)
{
	...
	request, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			node->nlogger.debug (nano::log::tag::frontier_req_client, "Error while sending bootstrap request: {}", ec.message ());
		}
	},
...
void nano::frontier_req_client::receive_frontier ()
{
	...
	connection->socket->async_read (connection->receive_buffer, nano::frontier_req_client::size_frontier, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			node->nlogger.debug (nano::log::tag::frontier_req_client, "Invalid size: expected {}, got {}", nano::frontier_req_client::size_frontier, size_a);
		}
	});
}
...
void nano::frontier_req_client::received_frontier (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	if (!ec)
	{
		...
		{
			node->nlogger.debug (nano::log::tag::frontier_req_client, "Aborting frontier req because it was too slow: {} frontiers per second, last {}", blocks_per_sec, account.to_account ());
		...
		{
			node->nlogger.debug (nano::log::tag::frontier_req_client, "Received {} frontiers from {}", count, connection->channel->to_string ());
		}
		...
		else
		{
			if (count <= count_limit)
			{
				...
				node->nlogger.debug (nano::log::tag::frontier_req_client, "Bulk push cost: {}", bulk_push_cost);
			}
			else
			{
	...
	{
		node->nlogger.debug (nano::log::tag::frontier_req_client, "Error while receiving frontier: {}", ec.message ());
	}
}
...
void nano::frontier_req_server::send_finished ()
{
	...
	node->nlogger.debug (nano::log::tag::frontier_req_server, "Frontier sending finished");
...
void nano::frontier_req_server::no_block_sent (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::frontier_req_server, "Error sending frontier finish: {}", ec.message ());
	}
}
...
void nano::frontier_req_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.debug (nano::log::tag::frontier_req_server, "Error sending frontier pair: {}", ec.message ());
	}
}
