...
nano::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, nano::work_pool & work_a, nano::node_flags flags_a, unsigned seq) :
...
nano::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, nano::node_config const & config_a, nano::work_pool & work_a, nano::node_flags flags_a, unsigned seq) :
	...
	port_mapping (*this),
	rep_crawler (*this),
	vote_processor (checker, active, observers, stats, config, flags, nlogger, online_reps, rep_crawler, ledger, network_params),
	warmed_up (0),
	...
	generator{ config, ledger, wallets, vote_processor, history, network, stats, nlogger, /* non-final */ false },
	final_generator{ config, ledger, wallets, vote_processor, history, network, stats, nlogger, /* final */ true },
	...
	process_live_dispatcher{ ledger, scheduler.buckets, vote_cache, websocket }
{
...
bool nano::node::block_confirmed_or_being_confirmed (nano::transaction const & transaction, nano::block_hash const & hash_a)
{
...
std::unique_ptr<nano::store> nano::make_store (nano::nlogger & nlogger, boost::filesystem::path const & path, nano::ledger_constants & constants, bool read_only, bool add_db_postfix, nano::rocksdb_config const & rocksdb_config, nano::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, nano::lmdb_config const & lmdb_config_a, bool backup_before_upgrade)
{
	if (rocksdb_config.enable)
	{
		return std::make_unique<nano::rocksdb::store> (nlogger, add_db_postfix ? path / "rocksdb" : path, constants, rocksdb_config, read_only);
	}

	return std::make_unique<nano::lmdb::store> (nlogger, add_db_postfix ? path / "data.ldb" : path, constants, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, backup_before_upgrade);
}
